...
namespace HASM
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (args.Length < 3)
            {
                Console.WriteLine("Usage: hasm <input> -o <output>");
                return;
            }
            string input = args[0];
            ...
            string asm = new HAssembler(code).Assemble();
            //check if it exists, if not, create it
            if (!System.IO.File.Exists(output))
            {
                System.IO.File.Create(output).Close();
            }
            System.IO.File.WriteAllText(output, asm);
        }
    }
    ...
    public class HAssembler
    {
        ...
        public string Assemble()
        {
            ...
            catch (Exception e)
            {
                //get line number
                int lineNumber = 0;
                foreach (var line in tokenizer.ToString().Split('\n'))
                {
                    if (line.Contains(tokenizer.Peek()))
                    {
                        break;
                    }
                    lineNumber++;
